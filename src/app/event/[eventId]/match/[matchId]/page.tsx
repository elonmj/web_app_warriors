...
import { ModifyMatchResultPopup } from '@/app/components/ModifyMatchResultPopup';
...
export default function MatchResultPage({
  ...
  const [ratingChanges, setRatingChanges] = useState<{
    ...
    const handleModifySubmit = async (newScore: [number, number]) => {
        setError("");
        try {
            const response = await fetch('/api/matches/modify', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    score: {
                        player1Score: newScore[0],
                        player2Score: newScore[1]
                    }
                }),
            });
            ...
            if (!response.ok) {
                setError(data.error || 'Failed to modify match result');
                return;
            }

            setMatch(data); // Update the match state with the new data
            setShowModifyPopup(false); // Close the popup

        } catch (error) {
            setError('Failed to modify match result');
        }
    }
    ...
    const canSubmit = !isCompleted && match.player2.id.toString() !== 'BYE';
    const canModify = match.status === 'completed';
        ...
        <div className="container mx-auto max-w-4xl px-4 py-8">
            ...
            dark:bg-onyx-900 dark:border-onyx-800 dark:hover:border-amethyste-700">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className={`flex items-center gap-2 ${isCompleted ? "text-green-600 dark:text-green-400" : "text-onyx-500 dark:text-onyx-400"
                        }`}>
              ...
              dark:bg-amethyste-700 dark:hover:bg-amethyste-600 mr-2"
                        >
                            Submit Match Result
                        </button>
                    )}
            ...
            {showCompletedPopup && (
                <MatchResultPopup
                    ...
                    player1Name={match.player1.name || match.player1.id.toString()}
                    player2Name={match.player2.name || match.player2.id.toString()}
            ...
            {showAdminDialog && (
                <AdminPasswordDialog
                    ...
                    description="Please enter the admin password to modify the match result."
                    warningMessage="Modifying a match result will recalculate player ratings and rankings."
                />
            )}

            {/* Modify Match Result Popup */}
            {showModifyPopup && (
